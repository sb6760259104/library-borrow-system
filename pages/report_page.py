...
def export_pdf_thai(report_df, report_start, report_end):
    ...
    elements.append(Paragraph("รายงานการยืม–คืนหนังสือ", styles["Title"]))
    elements.append(
        Paragraph(
            f"ช่วงวันที่ {report_start.strftime('%d/%m/%Y')} ถึง {report_end.strftime('%d/%m/%Y')}",
            styles["Normal"]
        )
    )
...
def render_report():
    ...
    # --------- pdf
    # --- PDF (ภาษาไทย) ---
    pdf_data = export_pdf_thai(report_df, report_start, report_end)

    st.download_button(
        label="⬇️ ดาวน์โหลดรายงานผู้ยืม–คืน (PDF ภาษาไทย)",
        data=pdf_data,
        file_name="borrow_return_report_thai.pdf",
